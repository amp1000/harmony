...
export default function Harmony() {
  ...
  const reset = () => { if (!confirm("Reset Harmony? This clears ALL data.")) return; ["hm_profile","hm_contacts","hm_lang","hm_gender","hm_checkins","hm_meds","hm_medlog","hm_activity","hm_checkin_time","hm_alert_contact","hm_tts_provider","hm_tts_key","hm_tts_voice","hm_weekly","hm_faceid","hm_biometric_id","hm_trial_start","hm_subscribed","hm_card"].forEach(k => store.del(k)); window.location.reload(); };
  ...
  if (screen === "roleSelect") return (
    <div style={{ padding: "0 16px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ paddingTop: 70, textAlign: "center", marginBottom: 36 }}>
        <Avatar sz={88} />
        <h1 style={{ fontSize: 34, fontWeight: 700, letterSpacing: "-0.03em", marginTop: 18 }}>Welcome to Harmony</h1>
        <p style={{ fontSize: 17, color: C.text2, marginTop: 8 }}>Who is using this device?</p>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
        <button onClick={() => chooseMode("senior")} style={{ ...card, textAlign: "left", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 16, padding: 20 }}>
          <span style={{ fontSize: 38 }}>♡</span>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 18, fontWeight: 600 }}>This is my device</p>
            <p style={{ fontSize: 14, color: C.text2, marginTop: 2, lineHeight: 1.4 }}>I want a companion to talk to and to check in each day. (For the senior.)</p>
          </div>
          <span style={{ fontSize: 20, color: C.text3 }}>›</span>
        </button>
        <button onClick={() => chooseMode("family")} style={{ ...card, textAlign: "left", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 16, padding: 20 }}>
          <span style={{ fontSize: 38 }}>👨‍👩‍👧</span>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 18, fontWeight: 600 }}>I'm checking on a loved one</p>
            <p style={{ fontSize: 14, color: C.text2, marginTop: 2, lineHeight: 1.4 }}>See how my parent or relative is doing, from my own phone. (For family.)</p>
          </div>
          <span style={{ fontSize: 20, color: C.text3 }}>›</span>
        </button>
      </div>
      <p style={{ fontSize: 12, color: C.text3, textAlign: "center", paddingBottom: 30 }}>You can change this anytime.</p>
    </div>
  );
      ...
      <div style={{ minHeight: "100vh", paddingBottom: 40 }}>
        <div style={{ padding: "16px 16px 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: C.text2 }}>Hi {familyName || "there"} · here's how they're doing</p>
          </div>
          <button onClick={familyLogout} style={{ width: 36, height: 36, borderRadius: "50%", border: "none", background: C.card, color: C.text2, fontSize: 16, cursor: "pointer" }}>⏻</button>
        </div>
        <div style={{ padding: "0 16px" }}>
          {linkedSenior?.demo && (
            <div style={{ background: C.amberSoft, borderRadius: 12, padding: 12, marginBottom: 12 }}>
              <p style={{ fontSize: 13, color: C.amber, fontWeight: 600 }}>Demo view</p>
              <p style={{ fontSize: 12, color: C.text2, marginTop: 2 }}>Showing sample data. Connect the backend for live updates from their real device.</p>
            </div>
          )}
          ...
          <div style={{ ...card, marginBottom: 12, background: checkedToday || linkedSenior?.demo ? C.greenSoft : C.amberSoft, textAlign: "center", padding: 24 }}>
            ...
            <p style={{ fontSize: 20, fontWeight: 700 }}>{checkedToday ? `Checked in · ${lastMood || "Great"}` : linkedSenior?.demo ? "Checked in · Great" : "Not checked in yet"}</p>
            <p style={{ fontSize: 14, color: C.text2, marginTop: 4 }}>{checkedToday || linkedSenior?.demo ? "Everything looks good today" : `${sName} hasn't checked in yet today`}</p>
          </div>

          {/* Today details */}
          <div style={{ ...card, marginBottom: 12 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: C.text2, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 12 }}>Today</p>
            <Row label="Daily check-in" value={checkedToday ? `✓ ${lastMood || "Done"}` : linkedSenior?.demo ? "✓ Great" : "Not yet"} good={checkedToday || linkedSenior?.demo} />
            <Row label="Medications" value={linkedSenior?.demo ? "2/2 taken" : medsLocal.length ? `${medsTakenCount}/${medsLocal.length} taken` : "None set"} good={linkedSenior?.demo || (medsLocal.length > 0 && medsTakenCount === medsLocal.length)} />
            <Row label="Talked to Harmony" value={talked || linkedSenior?.demo ? "Yes ✓" : "Not yet"} good={talked || linkedSenior?.demo} last />
          </div>

          {/* 7-day */}
          <div style={{ ...card, marginBottom: 12 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: C.text2, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 12 }}>Last 7 days</p>
          ...
          {/* Recent activity */}
          <div style={{ ...card, marginBottom: 12 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: C.text2, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 12 }}>Recent activity</p>
            {(linkedSenior?.demo ? [
            ...
            {!linkedSenior?.demo && act.length === 0 && <p style={{ fontSize: 14, color: C.text2 }}>No activity yet today</p>}
          </div>

          {/* Quick call */}
          <button onClick={() => alert("In the full version this calls " + sName + " directly.")} style={{ ...bigBtn }}>📞 Call {sName}</button>
        </div>
      </div>
    );
  }
  ...
  if (screen === "welcome") return (
    <div style={{ padding: "0 16px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", gap: 16, paddingTop: 60 }}>
        <Avatar sz={96} />
        <h1 style={{ fontSize: 40, fontWeight: 700, letterSpacing: "-0.03em" }}>Harmony</h1>
        <p style={{ fontSize: 19, color: C.teal, fontWeight: 600, lineHeight: 1.3, maxWidth: 320 }}>Know they're okay,<br/>every single day.</p>
        <p style={{ fontSize: 16, color: C.text2, lineHeight: 1.5, maxWidth: 320 }}>A warm companion for them. Peace of mind for you.</p>
      </div>
      <div style={{ paddingBottom: 40, display: "flex", flexDirection: "column", gap: 12 }}>
        <button onClick={() => {
          // If a key is already set, do the warm conversational setup. Otherwise go straight into the app.
        ...
        }} style={{ ...bigBtn }}>Get Started</button>
        <button onClick={() => { setScreen("app"); setTab("family"); }} style={{ ...grayBtn, fontSize: 15 }}>I'm a family member →</button>
      </div>
    </div>
  );
  ...
  if (screen === "app" && !locked && trialExpired) return (
    <div style={{ padding: "0 16px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", gap: 16, paddingTop: 30 }}>
        <Avatar sz={84} />
        <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.02em" }}>Your free trial has ended</h1>
        <p style={{ fontSize: 16, color: C.text2, lineHeight: 1.5, maxWidth: 330 }}>We hope Harmony has been a comforting companion. Continue keeping your family connected for {PRICE} a month.</p>
        <div style={{ ...card, width: "100%", maxWidth: 340, textAlign: "left", marginTop: 8 }}>
          {["Daily check-ins with family alerts","Warm AI companion in a human voice","Medication reminders & tracking","Family wellbeing dashboard","Weekly summaries"].map((f, i) => (
      ...
      <div style={{ paddingBottom: 40, display: "flex", flexDirection: "column", gap: 10 }}>
        <button onClick={subscribeNow} style={{ ...bigBtn }}>Subscribe · {PRICE}/month</button>
        <p style={{ fontSize: 12, color: C.text3, textAlign: "center", lineHeight: 1.5 }}>Cancel anytime. {PRICE}/month after subscribing.<br/>Secure payment by Stripe.</p>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", paddingBottom: 0 }}>
      ...
      {tab === "talk" && (
        <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
          <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: 12, borderBottom: `1px solid ${C.sep}` }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 17, fontWeight: 600 }}>Harmony</p>
              <p style={{ fontSize: 13, color: speaking ? C.teal : C.text2 }}>{speaking ? "Speaking…" : listening ? "Listening…" : busy ? "Thinking…" : "Here for you ♡"}</p>
            </div>
            {speaking && <button onClick={stopAudio} style={{ padding: "6px 12px", borderRadius: 20, border: `1px solid ${C.sep}`, background: "transparent", color: C.text2, fontSize: 13, cursor: "pointer" }}>Stop</button>}
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
            {msgs.map((m, i) => <Bubble key={i} m={m} speak={speakFn} />)}
            {busy && <Typing />}
        ...
        <Page title="Today" sub={new Date().toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}>
          {!subscribed && daysLeft <= 2 && daysLeft > 0 && (
            <div onClick={() => setTab("settings")} style={{ ...card, marginBottom: 14, background: C.amberSoft, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 14, fontWeight: 600, color: C.amber }}>{daysLeft} {daysLeft === 1 ? "day" : "days"} left in your trial</p>
                <p style={{ fontSize: 13, color: C.text2 }}>{cardOnFile ? `Charges ${fmtDate(chargeDate)}` : "Tap to keep Harmony"} ›</p>
              </div>
            </div>
          )}
          {/* Check-in card */}
          <div style={{ ...card, marginBottom: 14, background: checkedInToday ? C.greenSoft : C.card }}>
            {checkedInToday ? (
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div><p style={{ fontSize: 18, fontWeight: 600 }}>You've checked in today</p><p style={{ fontSize: 14, color: C.text2 }}>Your family knows you're doing well</p></div>
              </div>
            ) : (
              <>
                <p style={{ fontSize: 19, fontWeight: 600, marginBottom: 4 }}>How are you today?</p>
                <p style={{ fontSize: 14, color: C.text2, marginBottom: 16 }}>Tap one to let your family know you're okay</p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {[{ e: "😊", m: "Great", c: C.green }, { e: "🙂", m: "Okay", c: C.teal }, { e: "😟", m: "Not great", c: C.amber }].map(o => (
                    <button key={o.m} onClick={() => doCheckIn(o.m)} style={{ padding: "16px 8px", borderRadius: 14, border: `1.5px solid ${C.sep}`, background: C.bg, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 600, color: o.c }}>{o.m}</span>
                    </button>
                  ))}
                </div>
              </>
          ...
          <div style={{ ...card, marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: meds.length ? 14 : 0 }}>
              <p style={{ fontSize: 18, fontWeight: 600 }}>💊 Medications</p>
              <button onClick={() => setScreen("manageMeds")} style={{ fontSize: 14, color: C.teal, background: "none", border: "none", cursor: "pointer", fontWeight: 500 }}>{meds.length ? "Edit" : "Add"}</button>
            </div>
            {meds.length === 0 ? (
              <p style={{ fontSize: 14, color: C.text2 }}>No medications added yet</p>
              ...
              return (
                <div key={med.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: `1px solid ${C.sep}` }}>
                  ...
                  {taken ? (
                    <span style={{ fontSize: 14, color: C.green, fontWeight: 600 }}>✓ Taken</span>
                  ) : (
                    <button onClick={() => logMed(med, true)} style={{ padding: "8px 16px", borderRadius: 20, border: "none", background: C.teal, color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Mark taken</button>
                  )}
                </div>
              );
            })}
      ...
      {tab === "settings" && (
        <Page title="Settings">
          ...
          <div style={{ ...card, marginBottom: 14, background: subscribed ? C.greenSoft : (daysLeft <= 2 ? C.amberSoft : C.tealSoft) }}>
            {subscribed ? (
              <>
                <p style={{ fontSize: 17, fontWeight: 600, marginBottom: 3 }}>✓ Subscribed</p>
                <p style={{ fontSize: 14, color: C.text2 }}>{PRICE}/month · renews {fmtDate(chargeDate)}. Thank you for trusting Harmony.</p>
              </>
            ) : (
              <>
                <p style={{ fontSize: 17, fontWeight: 600, marginBottom: 3 }}>{daysLeft} {daysLeft === 1 ? "day" : "days"} left in your free trial</p>
                <p style={{ fontSize: 14, color: C.text2, marginBottom: 12, lineHeight: 1.5 }}>
                  {cardOnFile
                    ? `Your card will be charged ${PRICE} on ${fmtDate(chargeDate)}. We'll remind you 2 days before, on ${fmtDate(reminderDate)}. Cancel anytime before then for free.`
                    : `Add a card now to keep Harmony after your trial. You won't be charged until ${fmtDate(chargeDate)}, and we'll remind you 2 days before.`}
                </p>
                {!cardOnFile
                  ? <button onClick={addCardForTrial} style={{ ...bigBtn }}>Add card · charged {fmtDate(chargeDate)}</button>
                  : <button onClick={subscribeNow} style={{ ...bigBtn }}>Subscribe now · {PRICE}/month</button>}
              </>
            )}
          </div>
          <div style={{ ...card, marginBottom: 14, padding: 0, overflow: "hidden" }}>
            <SettingRow icon="🎙" label="Voice & API keys" onClick={() => setScreen("setupVoice")} />
            <SettingRow icon="💊" label="Medications" onClick={() => setScreen("manageMeds")} />
            <SettingRow icon="📞" label="Trusted contacts" onClick={() => setScreen("manageContacts")} />
            <SettingRow icon="⏰" label={`Daily check-in time · ${fmtTime(checkInTime)}`} onClick={() => setScreen("checkinTime")} last />
          </div>
          {/* Family code to share */}
          <div style={{ ...card, marginBottom: 14, textAlign: "center" }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: C.text2, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 8 }}>👨‍👩‍👧 Family Code</p>
            <p style={{ fontSize: 32, fontWeight: 700, letterSpacing: "0.15em", color: C.teal, marginBottom: 8 }}>{store.get("hm_family_code") || "------"}</p>
            <p style={{ fontSize: 13, color: C.text2, lineHeight: 1.5 }}>Share this with family. They enter it in their Harmony app to see how you're doing and get alerts if you miss a check-in.</p>
          </div>
          {/* Face ID toggle */}
          <div style={{ ...card, marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 22 }}>👤</span>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 16, fontWeight: 500 }}>Face Unlock</p>
                <p style={{ fontSize: 13, color: C.text2 }}>{!biometricSupported ? "Not available on this device" : faceIdEnabled ? "On — Harmony locks when closed" : "Off — open without unlocking"}</p>
              </div>
              <button
          ...
          <div style={{ ...card, marginBottom: 14 }}>
            <p style={{ fontSize: 14, color: C.text2, marginBottom: 6 }}>Voice status</p>
            <p style={{ fontSize: 15, fontWeight: 500 }}>{ttsProvider === "elevenlabs" && ttsKey ? "✨ ElevenLabs — human voice" : "Built-in browser voice"}</p>
          </div>
          <button onClick={reset} style={{ ...grayBtn, color: C.red, fontSize: 15 }}>Reset all data</button>
        </Page>
      )}
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: C.card, borderTop: `1px solid ${C.sep}`, display: "flex", paddingBottom: "env(safe-area-inset-bottom, 8px)", paddingTop: 8, zIndex: 100 }}>
        {[
          ...
          <button key={t.id} onClick={() => { stopAudio(); setTab(t.id); if (t.id !== "talk") setMsgs(m => m); }} style={{ flex: 1, background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "4px 0", opacity: tab === t.id ? 1 : 0.45 }}>
            <span style={{ fontSize: 22 }}>{t.icon}</span>
            <span style={{ fontSize: 10, fontWeight: 500, color: tab === t.id ? C.teal : C.text2 }}>{t.label}</span>
          </button>
        ))}
      </div>
